// npx jest src/core/tools/__tests__/prependToFileTool.test.ts

import { describe, expect, it, jest, beforeEach } from "@jest/globals"

import { prependToFileTool } from "../prependToFileTool"
...
describe("prependToFileTool", () => {
	...
	describe("Basic functionality", () => {
		it("should prepend content to a new file", async () => {
			// Setup
		...
		it("should prepend content to an existing file", async () => {
			...
			// Verify
			expect(mockDiffViewProvider.open).toHaveBeenCalledWith("test.txt")
			expect(mockDiffViewProvider.update).toHaveBeenCalledWith("test content\nexisting content", true)
			// The tool adds its own newline between new and existing content
			expect(mockAskApproval).toHaveBeenCalled()
			expect(mockFileContextTracker.trackFileContext).toHaveBeenCalledWith("test.txt", "roo_edited")
		})
	})
