...
export async function prependToFileTool(
	cline: Cline,
	block: ToolUse,
	...
	try {
		if (block.partial) {
			...
			if (!relPath) {
				...
				pushToolResult(await cline.sayAndCreateMissingParamError("prepend_to_file", "path"))
			...
			if (!newContent) {
				cline.consecutiveMistakeCount++
				cline.recordToolError("prepend_to_file")
				pushToolResult(await cline.sayAndCreateMissingParamError("prepend_to_file", "content"))
				await cline.diffViewProvider.reset()
				return
			}
